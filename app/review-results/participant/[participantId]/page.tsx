...
export default async function ReviewParticipantResultsPage({
  params,
}: {
    ...
    <ReviewResultsShell>
      <div className="mx-auto max-w-6xl px-6 pb-10">
        ...
        <div className="grid gap-6 xl:grid-cols-3">
          ...
          <Card className="glass-card sunset-shadow border-0 xl:col-span-1">
            <CardContent className="p-6">
              <h3 className="text-xl font-semibold text-[var(--ink)]">Либовиц</h3>
              <div className="mt-4 grid grid-cols-3 gap-3">
                ...
                <MiniCard
                  label="Желание"
                  value={report.liebowitz?.participationTotal ?? "—"}
                />
              </div>
              <p className="mt-4 text-sm leading-7 text-[var(--ink-secondary)]">
                {report.liebowitz?.interpretation ?? "—"}
              </p>
            </CardContent>
          </Card>
          <ResultCard
            ...
            body={`${report.author?.profileLabel ?? "—"}${
              report.author?.interpretation ? `. ${report.author.interpretation}` : ""
            }`}
          />
        </div>
          ...
          <Card className="glass-card sunset-shadow border-0 xl:col-span-1">
            <CardContent className="p-6">
              <h3 className="text-xl font-semibold text-[var(--ink)]">
                Внутренние шкалы авторского опросника
              </h3>
        ...
        <div className="mt-8 grid gap-6">
          <ResponsesCard
            title="Ответы по ОСТиСФ"
            rows={report.responses.socialAnxiety.map(
          ...
          />
          <ResponsesCard
            title="Ответы по авторскому опроснику"
            rows={report.responses.author.map((item) => `${item.text} — ${item.value}`)}
          />
        </div>
      </div>
    </ReviewResultsShell>
      ...
      <CardContent className="p-6">
        ...
        <div className="mt-4 space-y-3">
          {rows.map((row) => (
            <div
              key={row}
              className="rounded-2xl bg-white/75 p-4 text-sm leading-7 text-[var(--ink-secondary)]"
            >
              {row}
            </div>
          ))}
