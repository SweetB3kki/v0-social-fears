  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="mb-8">
          <p className="text-sm uppercase tracking-[0.24em] text-[var(--sunset-rose)]">Групповой отчёт</p>
          <h1 className="mt-3 text-3xl font-semibold text-[var(--ink)]">{report.group.code}</h1>
          <p className="mt-3 text-sm leading-7 text-[var(--ink-secondary)]">
            Участников: {report.group.participantCount}. Завершённых тестирований: {report.group.completedCount}.
          </p>
        </div>
        ...
        <div className="mt-8 grid gap-4">
          {report.participants.map((participant) => (
            <Link key={participant.id} href={`/results/participant/${participant.id}`}>
              <Card className="glass-card sunset-shadow border-0 transition-transform hover:-translate-y-0.5">
                <CardContent className="grid gap-4 p-6 md:grid-cols-[1.15fr_repeat(4,minmax(0,1fr))] md:items-center">
                  <div>
                    <div className="text-xl font-semibold text-[var(--ink)]">{participant.displayName}</div>
                  ...
                  <SmallMetric label="Конфликтные зоны" value={participant.liebowitzConflictCount ?? "—"} />
                </CardContent>
              </Card>
            </Link>
          ))}
  ...
  return (
    <Card className="glass-card sunset-shadow border-0">
      <CardContent className="p-5">
        <div className="text-xs uppercase tracking-[0.18em] text-[var(--sunset-rose)]">{label}</div>
...
function SmallMetric({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-2xl bg-white/75 p-4">
      ...
      <div className="mt-2 text-sm font-semibold leading-6 text-[var(--ink)]">{value}</div>
    </div>
  );
}
